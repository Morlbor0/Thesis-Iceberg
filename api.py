...
class IcebergTable(Resource):
    ...
    def get(self, table_name):
        try:
            print(f"Attempting to read Iceberg table: iceberg.iceberg_db.{table_name}")
            df = spark.read.table(f"iceberg.iceberg_db.{table_name}")
...
class RenameTableColumn(Resource):        
    def post(self, table_name, old_column_name , new_column_name):
        try:
            print(f"Attempting to change column name in Iceberg table: {table_name}")
            spark.sql(f"ALTER TABLE iceberg.iceberg_db.{table_name} RENAME COLUMN {old_column_name} TO {new_column_name}")
            print(f"'{old_column_name}' column name changed to '{new_column_name}'")
    ...
    # THIS WORKS ONLY IF THE COLUMN NAME WE INSERTED IN THE URL IS THE SAME AS THE COLUMN NAME IN THE ICEBERG TABLE
    # THE FACT THAT WE HAVE TO INPUT THIS INFORMATION MAKES THIS FLEXIBLE
        
class GetUser(Resource):
    def get(self, table_name, id):
        try:
            df = spark.sql(f"SELECT * FROM iceberg.iceberg_db.{table_name} WHERE id = {id}")
            data = df.toPandas().to_dict(orient="records")
            return jsonify(data)
    ...
    # THIS ONLY WORKS IF THE COLUMN NAME IS 'id' (STATIC)
...
class IcebergTableHistory(Resource):
    def get(self, table_name):
        try:
            df = spark.sql(f"SELECT * FROM iceberg.iceberg_db.{table_name}.history")
            data = df.toPandas().to_dict(orient="records")
            return jsonify(data)
...
class IcebergTableAge(Resource):
    def get(self, table_name):
        try:
            df = spark.read.table(f"iceberg.iceberg_db.{table_name}").select("age")
            data = df.toPandas().to_dict(orient="records")
            return jsonify(data)
...
class NewColumn(Resource):
    def post(self, table_name, column_name, column_type):
        try:
            spark.sql(f"ALTER TABLE iceberg.iceberg_db.{table_name} ADD COLUMN {column_name} {column_type}")
...
class DeleteColumn(Resource):    
    def delete(self, table_name, column_name):
        try:
            spark.sql(f"ALTER TABLE iceberg.iceberg_db.{table_name} DROP COLUMN {column_name}")
...
class GetFromDate(Resource):
    def get(self, table_name, date):
        try:
            ...
            df = spark.sql(f"SELECT * FROM iceberg.iceberg_db.{table_name} WHERE added_at >= '{date}'")
            data = df.toPandas().to_dict(orient="records")
            return jsonify(data)
...
class GetColumn(Resource):
    def get(self, table_name, column):
        try:
            columns_set = set(spark.table(f"iceberg.iceberg_db.{table_name}").columns)

            if column in columns_set:
                query = spark.sql(f"SELECT {column} FROM iceberg.iceberg_db.{table_name}")
                data = query.toPandas().to_dict(orient="records")
                return jsonify(data)
            else:
                last_version_path = f"spark-warehouse/iceberg/iceberg_db/{table_name}/metadata/version-hint.text"
                with open(last_version_path, "r") as f:
                    last_version = f.read()
                metadata_path = f"spark-warehouse/iceberg/iceberg_db/{table_name}/metadata/v{last_version}.metadata.json"
                ...
                if column in column_name_versions:
                    ...
                    query = spark.sql(f"SELECT {last_schema_fields_of_interest[0]['name']} FROM iceberg.iceberg_db.{table_name}")
                    data = query.toPandas().to_dict(orient="records")
...
api.add_resource(RenameTableColumn, "/<string:table_name>/<string:old_column_name>/to/<string:new_column_name>")
...
api.add_resource(GetUser, "/<string:table_name>/<int:id>")
